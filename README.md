...
At its core is a fast, single-file **Rust** binary (14 crates, 685 tests,
...
- **Rust ratatui TUI** (primary) — 24 focus areas: chat, explorer, editor, model selector, settings, code review, PR review, git commit, ByteBot agent, collaboration hub, background tasks, worktrees, insights, provider health, performance dashboard, project analyzer, feature navigator and more. Six of them (voice interface, terminal assistant, performance profiler, custom models, learning mode, multi-language) are interface mockups with scripted content — they render, they do not listen or profile. The security auditor is not one: `Enter` walks the workspace and runs the same vulnerability scanner `xencode analyze` uses.
...
```bash
cd rust
cargo test                          # Full workspace suite (685 tests)
...
- Making the six remaining scripted TUI panels real: microphone input, a live
  profiler, a terminal assistant backed by a model call
