...
At its core is a fast, single-file **Rust** binary (13 crates, 630 tests,
...
- **Approval-gated agent tool loop** — the chat model can call 10 tools (`read_file`, `list_dir`, `search_files`, `write_file`, `edit_file`, `run_command`, `background_start/poll/stop`, `repo_advise`); file changes and shell commands stop at a modal prompt showing the exact diff or command line (`y` allow · `a` allow for the session · `n`/`Esc` deny), paths outside the workspace are refused in every mode, every answer is logged in the transcript, and `/rewind` puts the files back.
...
```bash
cd rust
cargo test                          # Full workspace suite (630 tests)
