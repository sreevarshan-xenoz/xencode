...
class SmartContextManager:
    ...
    def analyze_file_relevance(self, file_path: Path, query: str) -> float:
        ...
        try:
            ...
            relevance = (
                (keyword_score / len(query_words)) + file_type_boost + main_file_boost
            )
    ...
    def add_conversation_context(self, role: str, content: str, model: str = None):
        ...
        context_item = ContextItem(
            ...
            metadata={"role": role, "model": model, "length": len(content)},
            timestamp=time.time(),
        ...
        conversation_items = [
            item for item in self.context_items if item.type == "conversation"
        ]
    ...
    def get_context_for_query(self, query: str, max_tokens: int = None) -> str:
        ...
        for item in sorted(
            self.context_items, key=lambda x: x.relevance_score, reverse=True
        ):
    ...
    def get_context_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "estimated_tokens": sum(
                len(item.content.split()) for item in self.context_items
            ),
        }
