...
class MultiModelManager:
            ...
            for line in lines:
                if line.strip():
                    ...
                    if model_name in self.model_capabilities:
                        self.available_models[model_name] = self.model_capabilities[
                            model_name
                        ]
    ...
    def detect_query_type(self, query: str) -> QueryType:
        ...
        creative_score = sum(
            1 for keyword in creative_keywords if keyword in query_lower
        )
        analysis_score = sum(
            1 for keyword in analysis_keywords if keyword in query_lower
        )
        explanation_score = sum(
            1 for keyword in explanation_keywords if keyword in query_lower
        )
        ...
        for msg in recent_messages:
            if msg["type"] == "user":
                ...
                context_parts.append(
                    f"Assistant ({msg.get('model', 'unknown')}): {msg['content'][:100]}..."
                )
