...
class ModelStabilityManager:
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        self.fallback_chains = {
            "code": ["codellama:7b", "qwen3:4b", "llama2:7b", "mistral:7b", "qwen:0.5b"],
        ...
        try:
            if self.state_file.exists():
                ...
                for model_name, state_data in data.items():
                    ...
                    state.avg_response_time_ms = state_data.get('avg_response_time_ms', 0)
                    ...
                    state.consecutive_query_count = state_data.get('consecutive_query_count', 0)
                    
                    # Parse datetime fields
                    if state_data.get('last_tested'):
                        state.last_tested = datetime.fromisoformat(state_data['last_tested'])
                    if state_data.get('degraded_until'):
                        state.degraded_until = datetime.fromisoformat(state_data['degraded_until'])
    ...
    def test_model_stability(self, model_name: str) -> StabilityResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Update model state
                self._update_model_success(model_name, response_time_ms, memory_usage_mb)
                ...
                return StabilityResult(
                    ...
                    status=StabilityStatus.DEGRADED if oom_detected else StabilityStatus.HEALTHY
                )
            else:
                # HTTP error
    ...
    def check_journalctl_oom(self, model_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                return ("ollama" in log_content or 
                        model_name.lower() in log_content or
                        "out of memory" in log_content)
                ...
                for line in lines:
                    if "oom-kill" in line.lower() or "out of memory" in line.lower():
                        ...
                        if self._is_recent_log_entry(line, current_time, minutes=5):
                            ...
                            if ("ollama" in line.lower() or 
                                model_name.lower() in line.lower()):
                                return True
                    ...
                    try:
                        ...
                        for line in lines:
                            ...
                            oom_indicators = [
                                ...
                                "cannot allocate memory", "killed", "segmentation fault"
                            ]
                            
                            if any(indicator in line_lower for indicator in oom_indicators):
                                if (model_name.lower() in line_lower and 
                                    self._is_recent_log_entry(line, current_time, minutes=5)):
                                    return True
    ...
    def _is_recent_log_entry(self, log_line: str, current_time: datetime, minutes: int = 5) -> bool:
    ...
    def _update_model_success(self, model_name: str, response_time_ms: int, memory_usage_mb: int) -> None:
        ...
        if state.avg_response_time_ms == 0:
            ...
            state.avg_response_time_ms = int(0.7 * state.avg_response_time_ms + 0.3 * response_time_ms)
    ...
    def get_fallback_chain(self, query_type: str) -> List[str]:
        ...
        return self.fallback_chains.get(query_type, self.fallback_chains["general"]).copy()
    ...
    def should_use_previous_model(self, model_name: str, query_context: Optional[str] = None) -> bool:
        """
    ...
    def get_model_health_summary(self) -> Dict[str, Dict]:
        ...
        for model_name, state in self.model_states.items():
            summary[model_name] = {
                'status': 'healthy' if state.is_stable and not state.is_degraded else 'degraded',
                ...
                'failure_count': state.failure_count,
                'consecutive_failures': state.consecutive_failures,
                'last_tested': state.last_tested.isoformat() if state.last_tested else None,
                'is_degraded': state.is_degraded,
                'degraded_until': state.degraded_until.isoformat() if state.degraded_until else None
            }
