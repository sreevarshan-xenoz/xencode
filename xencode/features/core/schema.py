...
class SchemaField:
    ...
    def _validate_type(self, value: Any) -> tuple[bool, Optional[str]]:
        """Route validation to the correct type-specific handler"""
        type_validators = {
            SchemaType.STRING: self._validate_string,
            SchemaType.INTEGER: self._validate_number,
            SchemaType.FLOAT: self._validate_number,
            SchemaType.BOOLEAN: self._validate_boolean,
            SchemaType.ARRAY: self._validate_array,
            SchemaType.OBJECT: self._validate_object,
            SchemaType.ENUM: self._validate_enum,
        }
    ...
    def _validate_number(self, value: Any) -> tuple[bool, Optional[str]]:
        """Validate integer or float type"""
        if not isinstance(value, (int, float)) or isinstance(value, bool):
            type_name = "an integer" if self.type == SchemaType.INTEGER else "a number"
            return False, f"Field '{self.name}' must be {type_name}"
    ...
    def _validate_array(self, value: Any) -> tuple[bool, Optional[str]]:
        """Validate array type"""
