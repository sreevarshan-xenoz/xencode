        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                # Check for exec/eval usage
                    ...
                    if hasattr(node.value.func, 'id') and node.value.func.id in [
                        'exec',
                        'eval',
                    ]:
                        violations.append(
    ...
    def scan_for_security_risks(
        ...
        for file_path in files:
            try:
                ...
                if any(
                    pattern in filename
                    for pattern in ['.env', 'secret', 'password', '.key', '.pem']
                ):
                    ...
                    if content_violations:
                        ...
                        for violation in content_violations:
                            if violation.violation_type == ViolationType.SENSITIVE:
                                ...
                                ViolationType.EXECUTABLE,
                                ViolationType.OBFUSCATED,
                            ]:
                                report.executables.append(file_path)
                                report.total_excluded += 1
    ...
    def generate_security_summary(self, report: SecurityReport) -> str:
        ...
        if report.total_excluded > 0:
            summary_parts.append(
                f"❌ Security alert: Excluded {report.total_excluded} risky files"
            )
        ...
        critical_violations = [
            v for v in report.violations if v.risk_level == RiskLevel.CRITICAL
        ]
        if critical_violations:
            summary_parts.append(
                f"🚨 {len(critical_violations)} critical security issues found"
            )
...
def main():
    ...
    test_paths = ["/tmp/test", "../etc/passwd", "normal/path", "/etc/passwd"]
