...
class PerformanceThrottler:
    ...
    def adjust_scan_strategy(
        self, base_strategy: ScanStrategy, current_usage: ResourceUsage
    ) -> ScanStrategy:
        """
...
class ProgressReporter:
    ...
    def format_progress_display(self, report: ProgressReport) -> str:
        ...
        lines.append(
            f"Size: {report.current_size_mb:.1f}/{report.total_size_mb:.1f} MB"
        )

        # Resource usage
        lines.append(
            f"Memory: {report.memory_usage_mb} MB | CPU: {report.cpu_usage_percent:.1f}%"
        )
...
class ResourceMonitor:
    ...
    def monitor_resource_usage(self) -> ResourceUsage:
        ...
        try:
            ...
            # Add to history
            self._usage_history.append({'timestamp': time.time(), 'usage': usage})
    ...
    def get_recommended_feature_set(self) -> Dict[str, bool]:
        ...
        recommendations = {
            ...
            'semantic_compression': self.system_profile.feature_level
            != FeatureLevel.BASIC,
            'parallel_processing': self.system_profile.cpu_cores > 2,
            'background_monitoring': self.system_profile.feature_level
        ...
        if current_usage.pressure_level in [
            ResourcePressure.HIGH,
            ResourcePressure.CRITICAL,
        ]:
    ...
    def adjust_strategy_for_pressure(
        self, current_strategy: ScanStrategy
    ) -> ScanStrategy:
