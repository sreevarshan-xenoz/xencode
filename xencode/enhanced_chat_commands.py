...
class EnhancedChatCommands:
    ...
    def handle_chat_command(
        self, command: str, args: str, current_model: str
    ...
    def handle_analyze_command(self, args: str) -> str:
        ...
        try:
            ...
            if os.path.isfile(path):
                ...
                all_issues.sort(
                    key=lambda x: severity_order.get(x[1].severity.value, 4)
                )
    ...
    def handle_model_command(
        self, args: str, current_model: str
    ) -> Tuple[str, Optional[str]]:
        ...
        if not args.strip():
            ...
            if not self.features.multi_model:
                return (
                    ...
                    + current_model
                    + "\n"
                    + "❌ Multi-model system not available for enhanced model management"
                ), None
            ...
            try:
                ...
                if not multi_model:
                    return (
                        f"📦 Current Model: {current_model}\n❌ Multi-model system not initialized",
                        None,
                    )
        ...
        if not self.features.multi_model:
            return (
                f"❌ Model switching requires Multi-Model System\nStaying with {current_model}",
                None,
            )
        ...
        try:
            ...
            if not stability_result.is_stable:
                return (
                    f"❌ Model {new_model} is not stable or available\n"
                    f"Error: {stability_result.error_message or 'Unknown stability issue'}\n"
                    f"Staying with {current_model}"
                ), None
    ...
    def handle_models_command(self) -> str:
        ...
        if not self.features.multi_model:
            return (
                "❌ Enhanced model listing requires Multi-Model System\n"
    ...
    def handle_context_command(self, args: str) -> str:
        ...
        if not args.strip():
            ...
            if not self.features.smart_context:
                return (
                    "❌ Context display requires Smart Context System\n"
        ...
        if subcommand == "clear":
            ...
            try:
                ...
                context_info = smart_context.scan_project_context(
                    current_dir, force_refresh=True
                )
    ...
    def handle_smart_command(self, args: str) -> str:
        ...
        if not (self.features.multi_model and self.features.smart_context):
            return (
                "❌ Smart mode requires both Multi-Model and Smart Context systems\n"
    ...
    def suggest_model_for_query(
        self, query: str, current_model: str
    ) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            if (
                current_time - self.last_query_time < 3.0
