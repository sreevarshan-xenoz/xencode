...
class EnhancedModelManager:
    ...
    def switch_model_with_stability_check(self, new_model: str) -> Tuple[bool, str]:
        ...
        if result.is_stable:
            ...
            if result.oom_detected:
                ...
                self.stability_manager.mark_model_degraded(new_model, duration_minutes=10)
    ...
    def execute_query_with_stability(self, query: str, query_type: str = "general") -> str:
        """
        ...
        if result.is_stable:
            ...
            print(f"📊 Performance: {result.response_time_ms}ms, {result.memory_usage_mb}MB")
        else:
            print(f"❌ Model {self.current_model} failed during query: {result.error_message}")
            
            if result.oom_detected:
                print(f"🚨 OOM detected! Marking {self.current_model} as degraded")
                self.stability_manager.mark_model_degraded(self.current_model, duration_minutes=5)
            
            # Try fallback
            fallback_chain = self.stability_manager.get_fallback_chain(query_type)
            for fallback_model in fallback_chain:
                if (fallback_model != self.current_model and 
                    self.stability_manager.is_model_available(fallback_model)):
                ...
                response = f"❌ All models failed, using emergency response"
    ...
    def get_system_status(self) -> dict:
        ...
        status = {
            ...
            'healthy_models': sum(1 for h in health_summary.values() if h['status'] == 'healthy'),
            'degraded_models': sum(1 for h in health_summary.values() if h['status'] == 'degraded'),
            'emergency_model': self.stability_manager.get_emergency_model(),
...
def demo_integration_scenarios():
    ...
    manager.stability_manager.mark_model_degraded(current_model, duration_minutes=1/60)
    ...
    degraded_models = [name for name, health in status['health_details'].items() 
    ...
    if degraded_models:
        ...
        for model in degraded_models:
            ...
            print(f"  • {model}: score {health['stability_score']:.2f}, "
...
def demo_chat_mode_integration():
    ...
    for i, (query, query_type) in enumerate(chat_queries, 1):
        ...
        if i == 6:  # Quick follow-up
            print(f"   ⚡ 3-second rule applied: using previous model")
