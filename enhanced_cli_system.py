...
class FeatureDetector:
    ...
    def detect_features(self, force_refresh: bool = False) -> FeatureAvailability:
        ...
        if (not force_refresh and 
            ...
            current_time - self.last_detection_time < self.cache_ttl_seconds):
...
Examples:
  ...
  xencode --git-commit
        ...
        parser.add_argument(
            'query',
            nargs='?',
            help='Query to send to AI model (legacy mode)'
        )
    ...
    def handle_models_command(self) -> str:
        ...
        if not self.features.multi_model:
            return ("❌ Multi-model system not available\n"
    ...
    def handle_context_command(self) -> str:
        ...
        if not self.features.smart_context:
            return ("❌ Smart context system not available\n"
    ...
    def _handle_feature_status(self) -> str:
        ...
        status_lines = [
            ...
            "",
            "Core Systems:",
            f"  ✅ Security Manager: Available",
            f"  ✅ Context Cache: Available", 
            f"  ✅ Model Stability: Available",
            "",
            "Enhanced Systems:"
        ]
        
        # Enhanced feature status
        status_lines.append(f"  {'✅' if self.features.multi_model else '❌'} Multi-Model System: {'Available' if self.features.multi_model else 'Not Available'}")
        status_lines.append(f"  {'✅' if self.features.smart_context else '❌'} Smart Context System: {'Available' if self.features.smart_context else 'Not Available'}")
        status_lines.append(f"  {'✅' if self.features.code_analysis else '❌'} Code Analysis System: {'Available' if self.features.code_analysis else 'Not Available'}")
