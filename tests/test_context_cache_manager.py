...
class TestContextCacheManager(unittest.TestCase):
    ...
    def test_context_data_serialization(self):
        ...
        self.assertEqual(
            restored_context.project_hash, self.sample_context.project_hash
        )
        self.assertEqual(
            restored_context.project_root, self.sample_context.project_root
        )
    ...
    def test_basic_save_and_load(self):
        ...
        # Save context
        success = self.cache_manager.save_context(
            self.test_project_hash, self.sample_context
        )
    ...
    def test_lock_timeout_handling(self):
        ...
        old_timestamp = datetime.now() - timedelta(
            seconds=self.cache_manager._lock_timeout_seconds + 10
        )
        lock_file = self.cache_manager._get_lock_file_path(self.test_project_hash)
    ...
    def test_cache_stats(self):
        ...
        self.assertEqual(
            stats['cache_base_dir'], str(self.cache_manager.cache_base_dir)
        )
    ...
    def test_concurrent_access_simulation(self):
        ...
        def worker_save(worker_id):
            try:
                ...
                success = self.cache_manager.save_context(
                    f"concurrent_test_{worker_id}", context
                )
    ...
    def test_same_project_concurrent_access(self):
        ...
        def worker_same_project(worker_id):
            try:
                ...
                success = self.cache_manager.save_context(
                    self.test_project_hash, context
                )
                results.append((worker_id, success))
    ...
    def test_edge_case_empty_context_data(self):
        ...
        empty_context = ContextData(project_hash="empty_test", project_root="/empty")
...
class TestConcurrentProcessAccess(unittest.TestCase):
    ...
    def worker_process(self, worker_id, results_queue):
        ...
        try:
            ...
            success = cache_manager.save_context(
                f"{self.test_project_hash}_{worker_id}", context
            )
    ...
    def test_multiprocess_access(self):
        ...
        for i in range(3):
            process = multiprocessing.Process(
                target=self.worker_process, args=(i, results_queue)
            )
