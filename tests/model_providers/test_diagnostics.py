...
class TestProviderDiagnostics:
    ...
    async def test_test_all_providers(self, diagnostics):
        ...
        # Mock all three providers using nested with statements
        # (Python 3.8 compatible — parenthesized with was added in 3.9)
        with patch.object(diagnostics, 'test_qwen') as mock_qwen:
            with patch.object(diagnostics, 'test_openrouter') as mock_openrouter:
                with patch.object(diagnostics, 'test_ollama') as mock_ollama:
