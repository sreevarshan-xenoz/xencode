...
from enhanced_cli_system import (
    EnhancedXencodeCLI, FeatureDetector, CommandRouter, 
    FeatureAvailability
)
from resource_monitor import (
    ResourceMonitor, HardwareProfiler, SystemProfile, 
    ResourceUsage, ScanStrategy, FeatureLevel, ResourcePressure
)
...
class TestHardwareProfiler(unittest.TestCase):
    ...
    def test_system_profiling(self):
        ...
        self.assertIn(profile.feature_level, [FeatureLevel.BASIC, FeatureLevel.STANDARD, FeatureLevel.ADVANCED])
...
class TestResourceMonitor(unittest.TestCase):
    ...
    def test_resource_usage_monitoring(self):
        ...
        self.assertIn(usage.pressure_level, [
    ...
    def test_feature_recommendations(self):
        ...
        expected_keys = [
            'context_scanning', 'semantic_compression', 'parallel_processing',
            'background_monitoring', 'detailed_progress', 'model_stability_testing'
        ]
...
class TestEnhancedCLI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('enhanced_cli_system.MULTI_MODEL_AVAILABLE', False), \
             patch('enhanced_cli_system.SMART_CONTEXT_AVAILABLE', False), \
...
class TestColdStartOptimization(unittest.TestCase):
    ...
    def test_background_initialization(self):
        ...
        with patch('enhanced_cli_system.MULTI_MODEL_AVAILABLE', True), \
             patch('enhanced_cli_system.SMART_CONTEXT_AVAILABLE', True), \
             patch('enhanced_cli_system.CODE_ANALYSIS_AVAILABLE', True):
            
            # Mock the Phase 1 systems
            with patch('enhanced_cli_system.MultiModelManager') as mock_mm, \
                 patch('enhanced_cli_system.SmartContextManager') as mock_sc, \
                 patch('enhanced_cli_system.CodeAnalyzer') as mock_ca:
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_graceful_degradation_scenario(self):
        ...
        with patch('enhanced_cli_system.MULTI_MODEL_AVAILABLE', True), \
             patch('enhanced_cli_system.MultiModelManager', side_effect=Exception("Import failed")):
    ...
    def test_resource_pressure_adaptation(self):
        ...
        with patch.object(monitor, 'monitor_resource_usage') as mock_usage:
            ...
            self.assertGreaterEqual(adjusted_strategy.pause_between_batches_ms, 
