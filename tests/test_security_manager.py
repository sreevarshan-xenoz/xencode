...
from security_manager import (
    SecurityManager, SecurityViolation, SecurityReport,
...
class TestSecurityManager(unittest.TestCase):
    ...
    def test_validate_project_path_invalid(self):
        ...
        # Non-existent path
        self.assertFalse(self.security_manager.validate_project_path("/nonexistent/path"))
    ...
    def test_sanitize_file_path_valid(self):
        ...
        sanitized = self.security_manager.sanitize_file_path(test_file, self.project_root)
    ...
    def test_sanitize_file_path_invalid(self):
        ...
        sanitized = self.security_manager.sanitize_file_path(outside_file, self.project_root)
        self.assertIsNone(sanitized)
        
        # Path traversal attempt
        traversal_path = os.path.join(self.project_root, "../../../etc/passwd")
        sanitized = self.security_manager.sanitize_file_path(traversal_path, self.project_root)
        self.assertIsNone(sanitized)
    ...
    def test_detect_symlink_risks_safe(self):
        ...
        violation = self.security_manager.detect_symlink_risks(symlink_file, self.project_root)
...
"""
        ...
        violations = self.security_manager.validate_file_content("test.py", safe_content)
...
"""
        ...
        violations = self.security_manager.validate_file_content("malicious.py", dangerous_content)
...
"""
        ...
        violations = self.security_manager.validate_file_content("config.py", sensitive_content)
        self.assertGreater(len(violations), 0)
    ...
    def test_validate_file_content_binary(self):
        ...
        violations = self.security_manager.validate_file_content("binary.exe", binary_content)
        self.assertGreater(len(violations), 0)
...
"""
        ...
        violations = self.security_manager.analyze_python_ast("dangerous.py", dangerous_code)
        self.assertGreater(len(violations), 0)
        
        # Should detect multiple dangerous patterns
        high_risk_violations = [v for v in violations if v.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]]
...
"""
        ...
        self.assertTrue(any(v.violation_type == ViolationType.OBFUSCATED for v in violations))
    ...
    def test_validate_git_directory_risky(self):
        ...
        self.assertTrue(any(v.violation_type == ViolationType.GIT_EXPLOIT for v in violations))
    ...
    def test_scan_for_security_risks_comprehensive(self):
        ...
        report = self.security_manager.scan_for_security_risks(test_files, self.project_root)
    ...
    def test_is_safe_to_scan(self):
        ...
        # Test safety checks
        self.assertTrue(self.security_manager.is_safe_to_scan(safe_file, self.project_root))
        self.assertFalse(self.security_manager.is_safe_to_scan(exe_file, self.project_root))
        self.assertFalse(self.security_manager.is_safe_to_scan("/etc/passwd", self.project_root))
...
class TestSecurityManagerIntegration(unittest.TestCase):
    ...
    def test_real_world_python_project(self):
        ...
        report = self.security_manager.scan_for_security_risks(all_files, self.project_root)
